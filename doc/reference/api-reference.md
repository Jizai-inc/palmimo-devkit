...
The remaining steps still run, the driver disconnect still cuts torque, and only
then is it re-raised (the first one, if several arrive). This mirrors
`connect()`'s own `BaseException` rollback: an interrupt is not an `Exception`,
