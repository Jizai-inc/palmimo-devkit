"""Tests for :mod:`palmimo_companion_agent.pipeline.vad` (no hardware, no ONNX, no network).
...
from palmimo_companion_agent.pipeline.vad import (
    Int16Array,
    UtteranceSegmenter,
...
def test_vad_load_downloads_and_verifies_sha256(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    from palmimo_companion_agent.pipeline import vad as vad_module
...
def test_segmenter_logs_utterance_started_and_finalized_on_silence(caplog: pytest.LogCaptureFixture) -> None:
    ...
    assert any("utterance started" in m for m in messages)
    assert any("utterance finalized" in m and "silence" in m for m in messages)


def test_segmenter_logs_finalized_with_max_cap_reason(caplog: pytest.LogCaptureFixture) -> None:
    caplog.set_level(logging.INFO)
    max_seconds = 1.0
    n_frames_for_max = int(max_seconds * 16000 / FRAME) + 5
    probs = [0.9] * n_frames_for_max
    ...
    assert result is not None
    messages = [record.message for record in caplog.records]
    assert any("utterance finalized" in m and "max-cap" in m for m in messages)


def test_segmenter_logs_idle_near_miss_summary_after_five_seconds(caplog: pytest.LogCaptureFixture) -> None:
    caplog.set_level(logging.INFO)
    n_frames = int(5.0 * 16000 / FRAME) + 1  # just over the 5-second idle window
    probs = [0.4] * n_frames
    vad = FakeVad(probs)
    seg = UtteranceSegmenter(vad=vad, start_threshold=0.5, end_threshold=0.35)

    for _ in range(n_frames):
        seg.feed(_chunk(FRAME))

    messages = [record.message for record in caplog.records]
    assert any("idle" in m and "max_prob=0.40" in m for m in messages)


def test_segmenter_does_not_log_idle_summary_when_quiet(caplog: pytest.LogCaptureFixture) -> None:
    caplog.set_level(logging.INFO)
    n_frames = int(5.0 * 16000 / FRAME) + 1
    probs = [0.05] * n_frames
    vad = FakeVad(probs)
    seg = UtteranceSegmenter(vad=vad, start_threshold=0.5, end_threshold=0.35)
    ...
    messages = [record.message for record in caplog.records]
    assert not any("idle" in m for m in messages)


def test_reset_also_resets_the_vad_recurrent_state() -> None:
    class RecordingVad:
        def __init__(self) -> None:
            self.resets = 0
    ...
    vad = RecordingVad()
    segmenter = UtteranceSegmenter(vad=vad, sample_rate=16000)
    segmenter.reset()
    assert vad.resets == 1


# --- the ONNX Runtime session policy (mirrors palmimo_wakeword_agent.vad) ---
...
class _FakeOrt:
    """The slice of the `onnxruntime` module this module touches."""
