...
def _toolset(fake: FakeToolset) -> AgentToolSet:
    return cast(AgentToolSet, fake)
...
async def test_wave_detection_waves_back_with_a_happy_face() -> None:
    toolset = FakeToolset()
    _, notify = _recorder()
    engine = ReflexEngine(_toolset(toolset), notify)
...
async def test_an_inhibited_engine_skips_the_reflex_entirely() -> None:
    ...
    toolset = FakeToolset()
    _, notify = _recorder()
    engine = ReflexEngine(_toolset(toolset), notify, inhibited=lambda: True)
...
async def test_inhibited_defaults_to_never_so_existing_callers_are_unaffected() -> None:
    toolset = FakeToolset()
    _, notify = _recorder()
    engine = ReflexEngine(_toolset(toolset), notify)  # no inhibited= given
